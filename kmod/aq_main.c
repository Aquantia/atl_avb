...
static int aq_ndev_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	...
	if (unlikely(aq_utils_obj_test(&aq_nic->flags, AQ_NIC_PTP_DPATH_UP)) /*&&
		 ...
		 * and PTPv2 IPv4 & IPv6 UDP.
		 */
		if (unlikely((((ip_hdr(skb)->version == 4) && (ip_hdr(skb)->protocol == IPPROTO_UDP)) ||
					(ipv6_hdr(skb)->version == 6)) &&
					(udp_hdr(skb)->dest == htons(PTP_EVENT_MESSAGE_PORT)))) {
...
static int aq_ndev_ioctl(struct net_device *netdev, struct ifreq *ifr, int cmd)
{
...
#ifdef TSN_SUPPORT
		case SIOCINITTSN:
			return aq_tsn_init(aq_nic, ifr);
		case SIOCRELEASETSN:
			return aq_tsn_release(aq_nic, false);
		case SIOCALLOCDMABUF:
			return aq_tsn_alloc_dma_buf(aq_nic, ifr);
		case SIOCFREEDMABUF:
			return aq_tsn_free_dma_buf(aq_nic, ifr);
		case SIOCLINKCMD:
			return aq_tsn_get_link(aq_nic, ifr);
#endif //TSN_SUPPORT
#ifdef PTM_SUPPORT
		case SIOCENPTM:
			return aq_ptp_ptm_enable(aq_nic->aq_ptp, ifr);
		case SIOCGETPTMINFO:
			return aq_ptp_read_ptm_info(aq_nic->aq_ptp, ifr);
#endif //PTM_SUPPORT
	case AQ_PTP_SYNC_CFG: {
			...
			return aq_ptp_configure_ext_gpio(netdev, &sync);
	}
#endif
	}
