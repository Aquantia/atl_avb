...
struct __packed hw_atl_info {
	...
	u32 setting_length;
	u32 caps_ex;
	enum gpio_pin_function gpio_pin[4];
	u32 pcie_aer_dump[17];
	u16 snr_margin[4];
};
...
struct aq_rx_filter_flex {
	...
	u32 filter_size;
	unsigned long byteen[4];
	u32 mask[128];
};

struct aq_rx_filter_l3 {
	u32 cmd;
	u8 location;
	u32 ip_dst[4];
	u32 ip_src[4];
...
enum {
	HW_ATL_RX_L2_DST_EN = 1u,
	HW_ATL_RX_L2_ET_EN = 2u,
	HW_ATL_RX_L2_UP_EN = 4u,
};

enum hw_atl_rx_ctrl_registers_flex {
	HW_ATL_RX_BYTE_A_APPLY             = BIT(0),
	HW_ATL_RX_BYTE_B_APPLY             = BIT(1),
	HW_ATL_RX_BIG_MASK_APPLY           = BIT(1),
	HW_ATL_RX_ENABLE_MNGMNT_QUEUE_FLEX = BIT(19),
	HW_ATL_RX_ENABLE_QUEUE_FLEX        = BIT(30),
	HW_ATL_RX_ENABLE_FLTR_FLEX         = BIT(31),
};
...
#define HW_ATL_RX_BOFFSET_QUEUE_FLEX     20U
#define HW_ATL_RX_BOFFSET_ACTION_FLEX    16U
