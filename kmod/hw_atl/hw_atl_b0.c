	...
	unsigned long byteen[4], u32 size, u32 filter)
{
	...
	hw_atl_rpf_flex_bytepat_wr_en_set(self, 0);
	for( i = 0; i < (size + sizeof(u32) - 1)/sizeof(u32); i++) {
		...
		if( (i & 0x7) == 0x7 || 
			...
			hw_atl_rpf_flex_mask_word_set(self, bytemask, filter, i >> 3);
			bytemask = 0;
		}
		hw_atl_rpf_flex_bytepat_data_set(self, ntohl(value));
		hw_atl_rpf_flex_bytepat_addr_set(self, i);
		hw_atl_rpf_flex_bytepat_sel_set(self, filter);
		hw_atl_rpf_flex_bytepat_wr_en_set(self, 1);
		hw_atl_rpf_flex_bytepat_wr_en_set(self, 0);
	}
}
...
const struct aq_hw_ops hw_atl_ops_b0 = {
	...
	.hw_filter_flex_set          = hw_atl_b0_hw_filter_flex_set,
