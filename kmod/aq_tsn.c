...
int aq_tsn_init(struct aq_nic_s *aq_nic, struct ifreq *ifr)
{
	...
	if (self) {// TSN is already initialised. Non-safe operation ? 
	...
	netdev_info(aq_nic->ndev, "Enable AVB/TSN Support.\n");
...
int aq_tsn_release(struct aq_nic_s *aq_nic, bool force)
{
	...
	if( !force && !atomic_dec_and_test(&self->ref_cnt) )
		return 0; //There are more than one connected client

	aq_nic->aq_tsn = NULL; // TODO Issue if there is a thread which sleeps before prev cond and next step
	if( aq_nic->aq_fw_ops->enable_tsn )
		aq_nic->aq_fw_ops->enable_tsn(aq_nic->aq_hw, 0);
	if( aq_nic->aq_hw_ops->hw_tsn_enable )
	...
	netdev_info(aq_nic->ndev, "Disable AVB/TSN Support. Ref %d.\n", atomic_read(&self->ref_cnt));
