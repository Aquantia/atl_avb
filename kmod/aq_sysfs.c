...
static int aq_create_bars(struct aq_tsn_s *self)
{
	...
	for (i = 0; mask && i < PCI_ROM_RESOURCE; mask >>= 1, i++){
		...
		if( pci_resource_flags(self->aq_nic->pdev, i) & IORESOURCE_MEM ) {
			...
			self->orig_flags[i] = res->flags;
			if (IORESOURCE_BUSY & res->flags) {
				res->flags &= ~IORESOURCE_BUSY;
				//printk("Fix resource %d child flag: old %x, new: %x\n", i, (uint32_t)self->orig_flags[i], (uint32_t)res->flags);
			}
			else {
				//printk("Resource %d child flag: %x\n", i, (uint32_t)res->flags);
			}
		}
		break;
	}
